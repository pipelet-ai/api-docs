...
The difference is that it uses a different queue endpoint, `wan22-svi-i2v`, and supports prompts per 5 seconds.
...
- Create request: `POST /fal/queue/wan22-svi-i2v`
- Check status: `GET /fal/queue/wan22-svi-i2v/requests/:requestId/status`
- Cancel request: `PUT /fal/queue/wan22-svi-i2v/requests/:requestId/cancel`
- Fetch result: `GET /fal/queue/wan22-svi-i2v/requests/:requestId`
...
```
curl -XPOST "https://api.pipelet.ai/fal/queue/wan22-svi-i2v?fal_webhook=https://echo.hetzner1.pipelet.ai/" -H "Authorization: Bearer ptk_1~kHCeA1uJgl3dPw.xxxx" -d '{
...
```
curl https://api.pipelet.ai/fal/queue/wan22-svi-i2v/requests/5e8f8ukijqeire/status -H "Authorization: Bearer {your-bearer-token}"
```

### 3) Cancel a queued request, Exactly the same as video generation

```
curl -XPUT https://api.pipelet.ai/fal/queue/wan22-svi-i2v/requests/5e8f8ukijqeire/cancel -H "Authorization: Bearer {your-bearer-token}"
```

### 4) Fetch the result, Exactly the same as video generation

```
curl -XGET https://api.pipelet.ai/fal/queue/wan22-svi-i2v/requests/5e8f8ukijqeire -H "Authorization: Bearer {your-bearer-token}"
```
